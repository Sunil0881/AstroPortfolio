      ...
      <div className="px-4 py-10">
        ...
        <div className="mb-5 space-y-5 md:space-y-0 block md:flex">
          <input
            ...
            className="p-2 w-full border rounded-md flex-grow"
          />
          <select
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {blogs.map((blog) => (
            <div key={blog._id} className="bg-white p-4 rounded-lg shadow-lg flex flex-col h-full">
              ...
              <div className="flex-grow my-1 md:h-6 font-semibold">Category: <span className='font-normal'>{blog.category}</span></div>
              <div className="text-gray-700 flex-grow overflow-hidden">
                <div
                  dangerouslySetInnerHTML={{ __html: blog.content.slice(0, 100) }}
                  className="text-gray-700 line-clamp-3 md:h-16"
                />
              </div>
              <div className="md:mt-auto my-3">
